...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <h3> Update Completed Status and Description</h3>
      <a href={`${API}/${todo.id}/completed/${todo.completed}`} className="btn btn-primary float-end">
        Update Todo Completed Status</a>
        <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
        <input type="checkbox" defaultChecked={todo.completed} onChange={(e)=>{setTodo({...todo,completed: !todo.completed})}}></input>
        <hr/>
        ...
        <input defaultValue={todo.id} className="form-control w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })}/>
        <input defaultValue={todo.description}  onChange={(e)=>{setTodo({...todo,description: e.target.value})}}></input>
    </div>
  );
}
